const notEmptyValidator = (value) => {
    // console.log('notEmptyValidator', value);
    return value.length !== 0;
}

export const CampaignNameField =     {
    label: 'Имя кампании',
    name: 'campaignName',
    type: 'text',
    required: true,
    defaultValue: 'mytestcampaign_01',
    desc: 'Допустимые символы: a-z,0-9,_',
    validator: notEmptyValidator,
};

export const CampaignSettintsFields  = [
    {
        label: 'msg-template',
        name: 'msg-template',
        type: 'text',
        required: true, // eslint-disable-next-line
        defaultValue: '${uk} напоминает вам, что за вашей квартирой по адресу ${adres} числится задолженность за жилищно-коммунальные услуги в размере ${dolg} рублей. Телефон ',
        desc: 'Шаблон сообщения',
        validator: notEmptyValidator,
    },
    {
        label: 'callerid',
        name: 'callerid',
        type: 'text',
        required: true,
        defaultValue: '79000000000',
        desc: 'Номер телефона, отображаемый у абонента (если позволяет транк)',
        validator: notEmptyValidator,
    },
    {
        label: 'Trunk',
        name: 'trunk',
        type: 'text',
        required: true,
        defaultValue: 'bee-mob-carousel',
        desc: 'Транк, через который делать исх. вызовы',
        validator: notEmptyValidator,
    },
    {
        label: 'channels',
        name: 'channels',
        type: 'text',
        required: true,
        defaultValue: '2',
        desc: 'Кол-во занимаемых каналов',
        validator: notEmptyValidator,
    },
    {
        label: 'interval-wtime',
        name: 'interval-wtime',
        type: 'text',
        required: true,
        defaultValue: '08:00:00-18:00:00',
        desc: 'Интервал времени, когда можно совершать звонки',
        validator: notEmptyValidator,
    },
    {
        label: 'interval-dow',
        name: 'interval-dow',
        type: 'text',
        required: true,
        defaultValue: 'mon-sun',
        desc: 'дни недели, когда можно совершать звонки',
        validator: notEmptyValidator,
    },
    {
        label: 'amount',
        name: 'amount',
        type: 'text',
        required: true,
        defaultValue: '1',
        desc: 'общее кол-во звонков, которое должно быть совершено',
        validator: notEmptyValidator,
    },
    {
        label: 'retry',
        name: 'retry',
        type: 'text',
        required: true,
        defaultValue: '3',
        desc: 'Общее кол-во повторов',
        validator: notEmptyValidator,
    },
    {
        label: 'retry-secs',
        name: 'retry-secs',
        type: 'text',
        required: true,
        defaultValue: '900',
        desc: 'Интервал (в секундах) повторного набора при неудачном вызове',
        validator: notEmptyValidator,
    },
    {
        label: 'interval-send',
        name: 'interval-send',
        type: 'text',
        required: true,
        defaultValue: '3600',
        desc: 'Интервал (в секундах) повторного набора при удачном вызове',
        validator: notEmptyValidator,
    },
    {
        label: 'hear-secs',
        name: 'hear-secs',
        type: 'text',
        required: true,
        defaultValue: '15',
        desc: 'Кол-во секунд, после прослушивания которых сообщение считается прослушанным. 0 - должно быть прослушано полностью',
        validator: notEmptyValidator,
    },
    {
        label: 'tts-voice',
        name: 'tts-voice',
        type: 'text',
        required: true,
        defaultValue: 'zahar',
        desc: 'Модель голоса',
        validator: notEmptyValidator,
    },
    {
        label: 'tts-speed',
        name: 'tts-speed',
        type: 'text',
        required: false,
        defaultValue: '0.9',
        desc: 'Скорость речи',
        validator: notEmptyValidator,
    },
    {
        label: 'tts-emotion',
        name: 'tts-emotion',
        type: 'text',
        required: false,
        defaultValue: 'evil',
        desc: 'Эмоция голоса',
        validator: notEmptyValidator,
    },
    {
        label: 'emails',
        name: 'emails',
        type: 'text',
        required: false,
        defaultValue: 'erofeev@regenergy.ru,zaharov.i@domkominvest.ru,nikonov.ae@domkominvest.ru,grishkova.te@domkominvest.ru,nurutdinov.rr@domkominvest.ru',
        desc: 'куда отправлять уведомления об основных событиях кампании(старт/стоп)',
        validator: notEmptyValidator,
    },
];
